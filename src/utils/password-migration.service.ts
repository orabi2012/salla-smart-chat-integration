...
export class PasswordMigrationService {
    ...
    constructor(
        ...
        private sallaStoreRepo: Repository<SallaStore>,
    ...
    async migrateExistingPasswords(): Promise<{
        ...
        const stores = await this.sallaStoreRepo.find();
        let encrypted = 0;
        let skipped = 0;
        let errors = 0;
        ...
        for (const store of stores) {
            try {
                ...
                if (this.passwordService.isPasswordEncrypted(store.ubiqfy_password)) {
                    skipped++;
                    this.logger.debug(`Store ${store.salla_store_name}: Password already encrypted, skipping`);
                    continue;
                }
                ...
                if (!store.ubiqfy_password || store.ubiqfy_password.trim() === '') {
                    skipped++;
                    this.logger.debug(`Store ${store.salla_store_name}: Empty password, skipping`);
                    continue;
                }
                ...
                await this.sallaStoreRepo.update(store.id, {
                    ubiqfy_password: this.passwordService.encryptPassword(originalPassword)
                });

                encrypted++;
                this.logger.log(`Store ${store.salla_store_name}: Password encrypted successfully`);

            } catch (error) {
                errors++;
                this.logger.error(`Store ${store.salla_store_name}: Failed to encrypt password - ${error.message}`);
            }
        }
    ...
    async verifyPasswordEncryption(): Promise<{
        ...
        for (const store of stores) {
            try {
                // Skip if password is empty
                if (!store.ubiqfy_password || store.ubiqfy_password.trim() === '') {
                    details.push({ storeName: store.salla_store_name, status: 'verified' });
                    verified++;
                    continue;
                }
                ...
                if (decryptedPassword && decryptedPassword.length > 0) {
                    verified++;
                    details.push({ storeName: store.salla_store_name, status: 'verified' });
                    this.logger.debug(`Store ${store.salla_store_name}: Password verification successful`);
                } else {
                    failed++;
                    details.push({
                        storeName: store.salla_store_name,
                        status: 'failed',
                ...
                this.logger.error(`Store ${store.salla_store_name}: Password verification failed - ${error.message}`);
            }
        }

...
export class SallaWebhookService {
    ...
    constructor(
        ...
        @InjectRepository(SallaStoreProduct)
        private readonly storeProductRepository: Repository<SallaStoreProduct>,
        @InjectRepository(SallaStoreProductOption)
        private readonly storeProductOptionRepository: Repository<SallaStoreProductOption>,
     ...
     */
    private async handleProductDeleted(store: SallaStore, productData: any): Promise<any> {
        this.logger.log(`🗑️  Product deleted in Salla: ${productData.name} (SKU: ${productData.sku})`);
        this.logger.log(`🔍 DEBUG: Looking for product with Salla ID: ${productData.id} (type: ${typeof productData.id})`);

        try {
            // Find options that match this Salla product ID since products are now stored at option level
            const storeProductOptions = await this.storeProductOptionRepository.find({
                where: {
                    salla_product_id: productData.id.toString()
                },
                relations: ['storeProduct', 'storeProduct.ubiqfyProduct']
            });

            this.logger.log(`🔍 DEBUG: Found ${storeProductOptions.length} options with salla_product_id = '${productData.id.toString()}'`);
            ...
            if (storeProductOptions && storeProductOptions.length > 0) {
                ...
                return {
                    ...
                    deleted_product: {
                        ...
                        salla_sku: productData.sku,
                        status: 'deleted_from_database'
                    },
                    removed_options: deletedOptions
                };
            } else {
                this.logger.warn(`⚠️  No linked product found for deleted Salla product: ${productData.sku}`);
                return {
     ...
     */
    private async handleAppInstalled(payload: SallaAppInstallationWebhook): Promise<any> {
        ...
        try {
            ...
            if (store) {
                ...
                store.salla_store_name = store.salla_store_name || `Store ${payload.merchant}`;
                ...
                try {
                    const deactivatedProducts = await this.storeProductRepository.find({
                        where: {
                            salla_store_id: updatedStore.id,
                            is_active: false
                        }
                    });
     ...
     */
    private async handleStoreAuthorization(payload: SallaStoreAuthorizationWebhook): Promise<any> {
        ...
        try {
            ...
            try {
                ...
                if (storeInfo) {
                    store.salla_store_name = storeInfo.name || `Store ${payload.merchant}`;
                    store.salla_owner_name = storeInfo.owner_name || 'Unknown';
                    store.salla_owner_email = storeInfo.owner_email || 'unknown@example.com';
            ...
            try {
                this.logger.log(`🔗 Registering webhooks for store: ${store.salla_store_name}`);
                ...
                this.logger.log(`✅ Webhooks registered successfully for store: ${store.salla_store_name}`);
            } catch (webhookError) {
    ...
    private async exportInitialSallaProducts(store: SallaStore, storeInfo?: any): Promise<void> {
        try {
            if (!store?.id) {
                this.logger.warn('⚠️ Store ID missing while attempting to export products');
                return;
            }

            const products = await this.sallaIntegrationService.getSallaProducts(store.id);

            if (!products || products.length === 0) {
                this.logger.log(`ℹ️ No products returned from Salla for store ${store.salla_store_name}; skipping export.`);
                return;
            }
            ...
            products.forEach((product) => {
                ...
                const normalizedSalePrice = this.extractPriceValue(product?.sale_price ?? product?.salePrice);
                const imageUrls = this.extractImageUrls(product?.images ?? product?.media ?? product?.gallery);
            ...
            const storeFolder = join(storesDataRoot, store.salla_store_id || store.id);
            ...
            try {
                ...
                this.logger.log(`♻️  Existing products.xlsx found for ${store.salla_store_name}; replacing with new export.`);
            } catch {
            ...
            const storeInfoData = storeInfo && Object.keys(storeInfo).length > 0
                ? storeInfo
                : {
            ...
            const storeInfoPayload = {
                ...
                url: storeInfoData?.domain || storeInfoData?.url || storeInfoData?.store_url || null,
                currency: storeInfoData?.currency || store.salla_currency,
                owner_name: store.salla_owner_name,
                owner_email: store.salla_owner_email,
                categories: Array.from(new Set(products.map((product) => product.category_id).filter(Boolean))),
            ...
            try {
                ...
                this.logger.log(`♻️  Existing store-info.json found for ${store.salla_store_name}; replacing with new export.`);
            } catch {
                // File does not exist; nothing to remove
            }
            await fs.writeFile(storeInfoPath, JSON.stringify(storeInfoPayload, null, 2), 'utf8');

            this.logger.log(`📊 Exported ${products.length} products & store info for ${store.salla_store_name} in ${storeFolder}`);
        } catch (error) {
            this.logger.error(`❌ Failed to export Salla products for store ${store?.salla_store_name ?? store?.id}: ${error.message}`);
        }
    }
    ...
    private extractPriceValue(price: any): number | string | '' {
        ...
        if (typeof price === 'object') {
            const candidate = price?.value ?? price?.amount ?? price?.current ?? price?.price ?? price?.selling_price;
     ...
     */
    private async handleAppUninstalled(payload: SallaAppUninstallationWebhook): Promise<any> {
        ...
        try {
            ...
            this.logger.log(`🔍 Found store to deactivate: ${store.salla_store_name} (ID: ${store.id})`);
    ...
    async getStoreForSetup(storeId: string): Promise<{ store: SallaStore }> {
        const store = await this.sallaStoreRepository.findOne({ where: { id: storeId } });
    ...
    async completeUbiqfySetup(storeId: string, setupData: {
        ...
        try {
            ...
            this.logger.log(`✅ Ubiqfy setup completed for store: ${store.salla_store_name}`);
